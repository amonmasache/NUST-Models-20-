...
dim(new.nust)
str(new.nust)
write.table(new.nust,"~/newnust.txt",sep="\t")
```
...
crps<-crps(actual_values, family = "normal", mean = mean(fcs_qrrf), sd = sd(fcs_qrrf))
mean(crps)
...
set.seed(201)
rf.all<-randomForest(GHI~., data = train_data)
summary(rf.all)
rf.all
...
#Using the r-squared value
# Make predictions on the testing set
fcs_rf<- predict(rf.all, newdata=test_data[, -which(names(test_data) == target_variable)])
r_squared.rf <- cor(fcs_rf, actual_values)^2

###Plotting the graph of the forecasts on the observed data
fcs_rf <- ts(fcs_rf)
accuracy(fcs_rf, actual_values)

win.graph()
plot(actual_values)
lines(fcs_rf,col="red")

######   CRPS EVALUATION   ######

crps<-crps(actual_values, family = "normal", mean = mean(fcs_rf), sd = sd(fcs_rf))
mean(crps)
```
